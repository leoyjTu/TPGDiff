...
def read_img(env, path, size=None):
    ...
    # dtype-aware normalization: convert to float32 in [0,1]
    if img.dtype == np.uint16:
        img = img.astype(np.float32) / 65535.0
    elif img.dtype == np.uint8:
        img = img.astype(np.float32) / 255.0
    else:
        # For float images (float32/float64) assume values already in [0,1];
        # just cast to float32. This avoids incorrect scaling for mixed inputs.

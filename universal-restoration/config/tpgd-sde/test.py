...
if prior_ckpt is None:
    raise ValueError("opt['path']['prior'] must be set to first-stage checkpoint for inference.")
...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            content_prior = prior_model.get_content_prior(img4clip)
            deg_embed = prior_model.encode_for_degradation(img4clip)
            content_context = content_prior.float()
            deg_context = deg_embed.float()

...
clip_model, preprocess = open_clip.create_model_from_pretrained('tpgd_ViT-B-32', pretrained=opt['path']['tpgd'])
...
def restore(image):
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        content_context, deg_context = clip_model.encode_image(img4clip, control=True)
        content_context = content_context.float()
        deg_context = deg_context.float()

    LQ_tensor = torch.tensor(image, dtype=torch.float32).permute(2, 0, 1).unsqueeze(0)
    noisy_tensor = sde.noise_state(LQ_tensor)
    model.feed_data(noisy_tensor, LQ_tensor, deg_context=deg_context, content_context=content_context)
    model.test(sde)
...
interface = gr.Interface(fn=restore, inputs="image", outputs="image", title="Image Restoration with TPGD", examples=examples)
interface.launch()
